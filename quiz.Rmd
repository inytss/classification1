...
For example, as an HR staff, we are instructed to investigate divisions that has a long history of an employee resigning and analyze their average monthly hours. Let's do some aggregation of `average_monthly_hours` for each division. Because you only focused on the employee who left, you should filter the historical data with the condition needed. You can use `filter()` then `group_by()` function by `division` and `summarise()` the mean of `average_montly_hours` and arrange it based on `average_montly_hours` from high to low using `arrange()` function.
