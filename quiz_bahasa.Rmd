...
Untuk menyelesaikan penilian ini, Anda diminta untuk membuat model klasifikasi yang digunakan untuk mengklasifikasikan karakteristik dari seorang karyawan yang mengundurkan diri atau tidak. Gunakan Logistic Regression dan k-Nearest Neighbor dengan mengikuti beberapa langkah berikut ini:

# Ekplorasi Data

Pertama, kita akan mulai dari menyiapkan dan melakukan ekplorasi pada data. Pada kuis ini, Anda akan menggunakan data pergantian karyawan (`turnover`). Data ini disimpan dalam bentuk `.csv` pada *repository* ini sebagai file `turnover_balance.csv`. Bacalah data tersebut menggunakan `read.csv()` atau `read_csv()` dan simpanlah sebagai objek `turnover`. Sebelum membuat model klasifikasi, Anda perlu melakukan analisis data ekplorasi untuk memahami data tersebut. Lihatlah struktur data `turnover` kita! Anda dapat memilih menggunakan fungsi `str()` atau `glimpse()`.

```{r}
# your code here
```

Data *turnover* terdiri dari 10 kolom dan 7.142 baris. Data ini merupakan data *human resource* yang menunjukkan data historis mengenai karakteristik karyawan yang akan mengundurkan diri atau tidak. Dibawah ini merupaka informasi lebih lanjut tentang variabel yang terdapat pda dataset:
